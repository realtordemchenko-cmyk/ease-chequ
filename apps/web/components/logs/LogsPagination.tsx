...
export default function LogsPagination({ logs, pageSize = 10 }: LogsPaginationProps) {
    ...
    const exportCSV = (scope: "current" | "all" = "current") => {
        ...
        const rows = target.map((l) =>
            [escapeCsv(l.id), escapeCsv(l.type), escapeCsv(l.message), escapeCsv(l.timestamp)].join(",")
        );
